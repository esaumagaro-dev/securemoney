...
export default function AdminDeposits() {
  ...
  useEffect(() => {
    const params: any = { page, per_page: perPage, type: "deposit" };
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-400 mt-1">Monitor and manage all deposit transactions</p>
        </div>
      ...
      <div className="bg-slate-900 rounded-2xl border border-slate-800 overflow-hidden">
        ...
        {txns.length === 0 && <p className="text-center py-12 text-slate-500">No deposits found</p>}
      </div>
      {totalPages > 1 && (

...
export default function BillPayments() {
  ...
  function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    toast.success("Bill payment initiated (demo)");
  }
    ...
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="max-w-lg mx-auto space-y-6">
      ...
      <Card padding="lg">
        <form onSubmit={handleSubmit} className="space-y-4">
          <Input label="Reference Number" type="text" value={ref} onChange={e => setRef(e.target.value)} placeholder="e.g. BILL-12345" required />
          <Input label="Amount" type="number" value={amount} onChange={e => setAmount(e.target.value)} placeholder="0.00" required />
          <Button type="submit" className="w-full" icon={<FileText className="w-4 h-4" />}>Pay Bill</Button>
        </form>
      </Card>
    </motion.div>
  );

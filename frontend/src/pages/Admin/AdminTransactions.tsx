...
export default function AdminTransactions() {
  ...
  const types = ["", "transfer", "deposit", "withdraw", "airtime", "bill_payment"];

  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
      <h1 className="text-2xl font-bold text-white">Transaction Management</h1>
      <div className="flex flex-wrap gap-3">
        <select value={typeFilter} onChange={e => { setTypeFilter(e.target.value); setPage(1); }} className="px-4 py-2 bg-slate-800 border border-slate-700 rounded-xl text-sm text-white focus:outline-none focus:ring-2 focus:ring-primary/50">
      ...
      <div className="bg-slate-900 rounded-2xl border border-slate-800 overflow-hidden">
        <table className="w-full text-sm">
          <thead><tr className="border-b border-slate-800">
            <th className="px-4 py-4 text-left text-slate-400 font-medium">Type</th>
          ...
          <tbody>
            {txns.map(tx => (
              <tr key={tx.id} className="border-b border-slate-800/50 hover:bg-slate-800/30">
                <td className="px-4 py-4 text-white capitalize">{tx.type.replace("_", " ")}</td>
        ...
        {txns.length === 0 && <p className="text-center py-12 text-slate-500">No transactions found</p>}
      </div>
      {totalPages > 1 && (
